...
const showAmpUpInputImageStep: StepDefinition = {
  ...
  systemPromptOld: "{{existing_image_url}}",
  systemPromptNew: "{{existing_image_url}}",
  outputType: "image_url",
};

// Step 3 (OLD flow only) — Refined Image. Display the row's pre-rendered
// amped URL verbatim. New flow skips this (it generates a fresh image
// downstream instead).
const showAmpUpRefinedImageStep: StepDefinition = {
  name: "show_refined_image",
  title: "Refined Image",
  description:
    "Old flow only — displays the row's pre-rendered amped image URL. New flow skips this cell (its refined image is generated by the Generate Image step instead).",
  model: "(no model — display-only)",
  provider: "none",
  renderOnly: true,
  diffOld: "same_as_old",
